...
import { stripeCustomers } from '@/src/db/schema';
// COMMENTED OUT: users import not needed while subscription sync is disabled
// import { stripeCustomers, users } from '@/src/db/schema';

// COMMENTED OUT: KV cache imports (causing connection errors)
...
export async function syncStripeDataToKV(customerId: string): Promise<StripeSubCache> {
  try {
    // COMMENTED OUT: Stripe subscription sync causing connection errors
 ...
 * COMMENTED OUT: Not being used while subscription sync is disabled
 */
...
export async function getSubscriptionStatus(stripeCustomerId: string): Promise<StripeSubCache> {
  try {
    // COMMENTED OUT: KV cache access causing connection errors
