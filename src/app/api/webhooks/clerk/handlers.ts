...
/**
 * NEW: Synchronize ERG group information to Clerk's publicMetadata
 */
async function synchronizeERGToClerk(clerkUserId: string, ergGroupName: string): Promise<void> {
  try {
    const client = await clerkClient();
    ...
    console.log('Webhook: Successfully synchronized ERG to Clerk', { clerkUserId, ergGroupName });
  } catch (error) {
    console.error('Webhook: Failed to synchronize ERG to Clerk', {
      clerkUserId,
      ergGroupName,
      error,
    });
...
/**
 * NEW: Associates a user with their employer and potentially a sponsored group
 */
async function associateUserWithEmployerAndSponsoredGroup(
  tx: DatabaseTransaction,
  user: typeof usersTable.$inferSelect,
  email: string,
...
/**
 * NEW: Associates a user with a specific sponsored group
 */
...
/**
 * NEW: Synchronize sponsored group information to Clerk's publicMetadata
 */
